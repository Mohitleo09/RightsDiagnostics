...
const heroContent = [
  {
    ...
    titleLine1: "Senior Wellness: Caring for Health,",
    titleLine2: "Together",
    description: "From routine screenings to tailored senior care, find the right health plans that nurture you and your partner's strength, comfort, and happiness. Because growing older together should mean living healthier, fuller lives.",
    buttonText: "Book Tests",
    image: "/Category/elder/elders1.jpg"
  },
  {
    id: 2,
    priorityText: "Comprehensive Care for Seniors",
    titleLine1: "Advanced Health Monitoring",
    titleLine2: "For a Better Tomorrow",
    description: "Our specialized programs offer continuous health monitoring and proactive care management to ensure seniors maintain their independence while staying healthy.",
    buttonText: "Learn More",
    image: "/elder2.jpg"
  },
  {
    id: 3,
    priorityText: "Expert Geriatric Support",
    titleLine1: "Personalized Healthcare",
    titleLine2: "Designed Just for You",
    description: "Experience healthcare tailored specifically to the unique needs of seniors with our team of geriatric specialists and personalized treatment plans.",
...
export default function EldersCategory() {
  ...
  const iconBgMap = {
    ...
    const imageMap = {
      '1': '/Category/elder/Nutrition.jpg',
      '2': '/Category/elder/Vision.jpg',
      '3': '/Category/elder/Bones.jpg',
      '4': '/Category/elder/Cognitive.jpg',
      '5': '/Category/elder/heartHealth.jpg',
      '6': '/Category/elder/Emotional.jpg'
    };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch tests for elders category
        const testsResponse = await fetch(`/api/tests?category=Elders&_t=${Date.now()}`);
        const testsData = await testsResponse.json();
        
        // Fetch packages for elders category
        const packagesResponse = await fetch(`/api/packages?category=Elders&_t=${Date.now()}`);
        const packagesData = await packagesResponse.json();
        
        // Fetch videos for elders category
        const videosResponse = await fetch('/api/videos?category=Elders');
        const videosData = await videosResponse.json();
        
        // Fetch FAQs for elders category
        const faqsResponse = await fetch('/api/faqs?category=Elders&status=Active');
          ...
          const focusAreasData = [
            {
              ...
              description: "A balanced diet rich in fiber, vitamins, and minerals aids digestion, supports immunity, and maintains healthy body weight.",
            },
            {
              id: 2,
              title: "Vision & Hearing Health",
              description: "Regular eye and ear checkups help detect early signs of age-related vision or hearing loss, ensuring better daily living and safety.",
            },
            {
              id: 3,
              title: "Bone & Joint Care",
              description: "Balanced nutrition, adequate calcium, and regular movement help maintain strong bones and flexible joints, lowering the risk of arthritis and fractures.",
            },
            {
              id: 4,
            ...
            },
            {
              id: 5,
              title: "Heart Health",
              description: "Routine heart checkups and balanced blood pressure and cholesterol levels support cardiovascular health and promote lasting energy and vitality.",
            },
            {
              id: 6,
              title: "Emotional & Social Well-being",
              description: "Staying connected with loved ones, managing stress, and seeking emotional support enhance mental peace and overall happiness.",
            }
          ];
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#A8C3A3] to-white via-white flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#007AFF] mx-auto"></div>
          <p className="mt-4 text-gray-600">Loading senior health data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-b from-[#A8C3A3] to-white via-white" style={{ 
        background: 'linear-gradient(to bottom, #A8C3A3 0%, #FFFFFF 48%)' 
      }}>
        <Navbar />
        {/* Hero Section */}
      ...
      <section id="focus-areas" className="py-12">
        <div className="container mx-auto px-2 sm:px-3 lg:px-4">
          <h2 className="text-2xl lg:text-4xl font-bold text-gray-900 mb-12 text-left">
            Key Focus Areas for Elders's Health
          </h2>
          ...
          <div className="flex flex-col lg:flex-row gap-12 items-start">
            ...
            <div className="lg:w-1/3 flex justify-center items-start pt-4">
              <img
                src="/Category/elder/elders2.png"
                alt="Senior couple"
      ...
      {/* Lab Test Options - Carousel */}
      <section id="lab-tests" className="py-8">
        <div className="container mx-auto px-2 sm:px-3 lg:px-4">
              ...
              <div className="embla__container flex">
                {labTestOptions.map((test) => (
                  <div className="embla__slide flex-shrink-0 w-1/4 px-2" key={test.id}>
                    <Card 
                      key={test.id} 
                      className="hover:shadow-xl transition-all hover:-translate-y-1 bg-[#CBECC5] rounded-xl p-6 h-full cursor-pointer"
                      onClick={() => {
              ...
              <div className="embla__container flex">
                {packages.map((pkg) => (
                  <div className="embla__slide flex-shrink-0 w-1/4 px-2" key={pkg.id}>
                    <Card className="hover:shadow-xl transition-all hover:-translate-y-1 bg-gradient-to-br from-[#CBECC5] to-[#CBECC5] text-white rounded-xl p-6 h-full">
                      <CardHeader>
                        {pkg.badge && (
      ...
      <section className="py-8">
        <div className="container mx-auto px-2 sm:px-3 lg:px-4">
          <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-4 text-center">
            The 2-Minute Senior Health Hub
          </h2>
                    ...
                    <div className="space-y-4">
                      {videos.map((video) => (
                        <div 
                          ...
                          className={`bg-[#CBECC5] rounded-lg p-4 cursor-pointer hover:shadow-md transition-all flex items-center space-x-4 border-l-4 ${
            ...
            {faqData.map((faq, index) => (
              <div key={faq._id || faq.id || index} className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                ...
                >
                  ...
                  <span className="text-2xl w-8 h-8 flex items-center justify-center rounded-full bg-[#A8C3A3] text-white leading-none flex-shrink-0">
                    {openFaqId === faq.id ? '−' : '+'}
                  </span>
                </button>
                {openFaqId === faq.id && (
