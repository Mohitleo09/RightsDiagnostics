...
const heroContent = [
  {
    ...
    titleLine1: "Couples’ Health: Thrive ",
    titleLine2: "Together in Wellness",
    description: "Explore tailored health packages crafted for you and your partner — from preventive screenings to holistic care plans — nurturing both physical vitality and emotional connection. Because a strong partnership begins with shared well-being.",
    buttonText: "Book Tests",
    image: "/Category/couples/couples1.jpg"
  },
...
export default function CouplesCategory() {
  ...
  const iconBgMap = {
    ...
    const imageMap = {
      '1': '/Category/couples/Cookies.png',
      '2': '/Category/couples/HeartHealth.png',
      '3': '/Category/couples/ManRunning.png',
      '4': '/Category/couples/Romance.png',
      '5': '/Category/couples/Brain.png',
      '6': '/Category/couples/Dna.png'
    };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch tests for couples category
        const testsResponse = await fetch(`/api/tests?category=Couples&_t=${Date.now()}`);
        const testsData = await testsResponse.json();
        
        // Fetch packages for couples category
        const packagesResponse = await fetch(`/api/packages?category=Couples&_t=${Date.now()}`);
        const packagesData = await packagesResponse.json();
        
        // Fetch videos for couples category
        const videosResponse = await fetch('/api/videos?category=Couples');
        const videosData = await videosResponse.json();
        
        // Fetch FAQs for couples category
        const faqsResponse = await fetch('/api/faqs?category=Couples&status=Active');
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#F25F5C] to-white via-white flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#007AFF] mx-auto"></div>
          <p className="mt-4 text-gray-600">Loading couples health data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-b from-[#A8C3A3] to-white via-white" style={{ 
        background: 'linear-gradient(to bottom, #F25F5C 0%, #FFFFFF 48%)' 
      }}>
        <Navbar />
        {/* Hero Section */}
      ...
      <section id="focus-areas" className="py-12">
        <div className="container mx-auto px-10">
          <h2 className="text-2xl lg:text-4xl font-bold text-gray-900 mb-12 text-left">
            Key Focus Areas for Couples' Health
          </h2>
          ...
          <div className="flex flex-col lg:flex-row gap-12 items-start">
            ...
            <div className="lg:w-1/3 flex justify-center items-start pt-4">
              <img
                src="/Category/couples/couple2.png"
                alt="Senior couple"
              ...
              <div className="embla__container flex">
                {labTestOptions.map((test) => (
                  <div className="embla__slide flex-shrink-0 w-1/4 px-2" key={test.id}>
                    <Card 
                      key={test.id} 
                      className="hover:shadow-xl transition-all hover:-translate-y-1 bg-[#FDBEBF] rounded-xl p-6 h-full cursor-pointer"
                      onClick={() => {
              ...
              <div className="embla__container flex">
                {packages.map((pkg) => (
                  <div className="embla__slide flex-shrink-0 w-1/4 px-2" key={pkg.id}>
                    <Card className="hover:shadow-xl transition-all hover:-translate-y-1 bg-gradient-to-br from-[#FDBEBF] to-[#FDBEBF] text-white rounded-xl p-6 h-full">
                      <CardHeader>
                        {pkg.badge && (
      ...
      <section className="py-8">
        <div className="container mx-auto px-10">
          <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-4 text-center">
            The 2-Minute Couples Health Hub
          </h2>
                    ...
                    <div className="space-y-4">
                      {videos.map((video) => (
                        <div 
                          ...
                          className={`bg-[#FDBEBF] rounded-lg p-4 cursor-pointer hover:shadow-md transition-all flex items-center space-x-4 border-l-4 ${
            ...
            {faqData.map((faq, index) => (
              <div key={faq._id || faq.id || index} className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                ...
                >
                  ...
                  <span className="text-2xl w-8 h-8 flex items-center justify-center rounded-full bg-[#F25F5C] text-white leading-none flex-shrink-0">
                    {openFaqId === faq.id ? '−' : '+'}
                  </span>
                </button>
                {openFaqId === faq.id && (
