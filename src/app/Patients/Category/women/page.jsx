...
const heroContent = [
  {
    ...
    titleLine1: "Women’s Health: Take ",
    titleLine2: "Charge of Your Well-Being",
    description: "From preventive care to comprehensive diagnostics, discover the right tests and health packages to support your unique needs. Because proactive care is the best care.",
    buttonText: "Book Tests",
    image: "/Category/women/women1.jpg"
  },
  {
    id: 1,
    priorityText: "Your health is our top priority",
...
export default function WomenCategory() {
  ...
  const iconBgMap = {
    ...
    const imageMap = {
      '1': '/Category/women/Heart.png',
      '2': '/Category/women/veganFood.png',
      '3': '/Category/women/Cancer.png',
      '4': '/Category/women/Uterus.png',
      '5': '/Category/women/mental.png',
      '6': '/Category/women/bone&muscular.png'
    };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch tests for women category
        const testsResponse = await fetch(`/api/tests?category=Women&_t=${Date.now()}`);
        const testsData = await testsResponse.json();
        
        // Fetch packages for women category
        const packagesResponse = await fetch(`/api/packages?category=Women&_t=${Date.now()}`);
        const packagesData = await packagesResponse.json();
        
        // Fetch videos for women category
        const videosResponse = await fetch('/api/videos?category=Women');
        const videosData = await videosResponse.json();
        
        // Fetch FAQs for women category
        ...
        const focusAreasData = [
          {
            id: 1,
            title: "Heart Health",
            description: "Cardiovascular disease is one of the leading causes of death among women. Maintaining a balanced diet, regular exercise, and stress management.",
          },
          {
            id: 2,
            title: " Nutrition & Digestive Health",
            description: "A nutrient-rich diet supports energy, hormonal health, and disease prevention. Iron, calcium, and vitamin D are especially important for women of all ages.",
          },
          {
            id: 3,
            title: "Cancer Prevention & Screening",
            description: "Regular screenings for breast, cervical, and ovarian cancers can save lives. Early detection through mammograms and Pap smears is key for effective treatment.",
          },
          {
            id: 4,
            title: "Reproductive & Sexual Health",
            description: "Covers menstrual health, fertility, contraception, pregnancy, and menopause. Regular gynecological checkups ensure early detection of reproductive issues and overall hormonal balance."
          },
          {
            id: 5,
            title: " Mental & Emotional Well-Being",
            description: "Depression, anxiety, and stress-related disorders are more common in women. Prioritizing self-care, mindfulness, and professional support strengthens emotional resilience.",
          },
          {
            id: 6,
            title: "Bone & Muscular Health",
            description: "Women are at higher risk of osteoporosis after menopause. Adequate calcium intake, vitamin D, and weight-bearing exercise help maintain strong bones and muscles.",
          }
        ];
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#F5A6C4] to-white via-white flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#007AFF] mx-auto"></div>
          <p className="mt-4 text-gray-600">Loading women's health data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-b from-[#A8C3A3] to-white via-white" style={{ 
        background: 'linear-gradient(to bottom, #F5A6C4 0%, #FFFFFF 48%)' 
      }}>
        <Navbar />
        {/* Hero Section */}
              ...
              <div className="embla__container flex">
                {labTestOptions.map((test) => (
                  <div className="embla__slide flex-shrink-0 w-1/4 px-2" key={test.id}>
                    <Card 
                      key={test.id} 
                      className="hover:shadow-xl transition-all hover:-translate-y-1 bg-[#FFD4E4] rounded-xl p-6 h-full cursor-pointer"
                      onClick={() => {
              ...
              <div className="embla__container flex">
                {packages.map((pkg) => (
                  <div className="embla__slide flex-shrink-0 w-1/4 px-2" key={pkg.id}>
                    <Card className="hover:shadow-xl transition-all hover:-translate-y-1 bg-gradient-to-br from-[#FFD4E4] to-[#FFD4E4] text-white rounded-xl p-6 h-full">
                      <CardHeader>
                        {pkg.badge && (
      ...
      <section className="py-8">
        <div className="container mx-auto px-10">
          <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-4 text-center">
            The 2-Minute Women's Health Hub
          </h2>
                    ...
                    <div className="space-y-4">
                      {videos.map((video) => (
                        <div 
                          ...
                          className={`bg-[#FFD4E4] rounded-lg p-4 cursor-pointer hover:shadow-md transition-all flex items-center space-x-4 border-l-4 ${
            ...
            {faqData.map((faq, index) => (
              <div key={faq._id || faq.id || index} className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                ...
                >
                  ...
                  <span className="text-2xl w-8 h-8 flex items-center justify-center rounded-full bg-[#F5A6C4] text-white leading-none flex-shrink-0">
                    {openFaqId === faq.id ? '−' : '+'}
                  </span>
                </button>
                {openFaqId === faq.id && (
